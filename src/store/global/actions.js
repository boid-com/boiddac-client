...
export async function connectScatter (
  ...
  const network = await state.networks.find(n => n.name == state.active_network)
  ...
  ScatterJS.scatter
    ...
    .then(async connected => {
      ...
      if (ScatterJS.wallet != 'Lynx') {
        ...
        const networkwithtoken = Object.assign({}, network, token)
...
export async function login ({ state, dispatch, rootGetters, commit }) {
  ...
  const account = await state.scatter.login().catch(e => {
    console.log(e)
    return false
  })
...
export async function getDacApi ({ state, commit }, rebuild = false) {
  ...
  const rpc = new JsonRpc(n.fullhost())
...
export async function getEosScatter ({ state, commit }, rebuild = false) {
  ...
  const eos = state.scatter.eos(network, Api, { rpc, beta3: true })
...
export async function switchNetwork (
  ...
  const network = state.networks.find(n => n.name == payload.networkname)
  if (!network) {
...
export async function testEndpoint ({ state, commit }, endpointurl = false) {
  if (!endpointurl) {
    const network = await state.networks.find(
