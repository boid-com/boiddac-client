...
export async function loggedInRoutine({ state, commit, dispatch }, account) {
  ...
  const [balance, termsversion, isCandidate] = await Promise.all(requests);
...
export async function fetchDacVotes(
  ...
  const accountN = accountname || state.accountName;
  const api = await dispatch("global/getDacApi", false, { root: true });
  const votes = await api.getVotes(accountN);
  if (!accountname) {
...
export async function fetchBalances(
  ...
  const [dacbalance, systembalance] = await Promise.all(requests);
  if (!accountname) {
...
export async function fetchPendingPay(
  ...
  const requests = [api.getPendingPay(accountN), api.getPendingPay2(accountN)];
...
export async function fetchCatDelegations(
  ...
  const accountN = accountname || state.accountName;
  const api = await dispatch("global/getDacApi", false, { root: true });
  const res = await api.getCatDelegations(accountN);
...
export async function transact(
  ...
  const account = (await rootState.global.scatter.login()).accounts[0]; // same as getIdentity
  ...
  try {
    const [eos] = await dispatch("global/getEosScatter", null, { root: true });
    setTimeout(() => {
...
export async function proposeMsig(
  ...
  const api = await dispatch("global/getDacApi", false, { root: true });
  // proposalname
  const proposal_name = payload.proposal_name || this._vm.$helper.randomName();
  // expiration
  const exp = new Date();
  exp.setDate(exp.getDate() + 30);
  const [expiration] = exp.toISOString().split(".");
  ...
  for (let i = 0; i < payload.actions.length; i++) {
    ...
    const hexdata = await api.serializeActionData(action);
    action.data = hexdata;
  ...
  const PERM = rootGetters["dac/getAuthAccountPermLevel"];
  ...
  const msig_actions = [propose];
  if (!payload.is_personal_msig) {
    msig_actions.push(proposed);
  }
  const res = await dispatch("transact", { actions: msig_actions });
  if (res) {
...
function parseError(err) {
  ...
  if (
    ...
    const t = "contract_errors." + err.split("::")[1];
    err = t;
  } else {
    if (err && err.error) {
